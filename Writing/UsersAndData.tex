...
Then with the assigned values we calculated a mean value that would measure users ease of disclosure of each data item in the contexts we defined. We also evaluated how users felt about disclosing different data items in the same context using Two way ANOVA with repetition. We considered the sensitivity of data items, and the relatedness of the data items for the particular context we defined when deriving these results. For example, if a user feels comfortable disclosing their blood type into a health related application that could imply that the user is comfortable because that particular data item is related to the functionality that application provides. However, at the same time the user might feel uncomfortable disclosing their previous medicines taken to the same application because they perceive it to be highly sensitive. We discuss these aspects in detail when we present our results

For the qualitative analysis of the descriptive answers we received for the open ended question we made use of the process of coding. One coder initially generated a self coding scheme by summarizing the answers we received. Then we used another coder to do the same for the data set. Both coding schemes were similar and interchangeable and hence the first coder then coded all answers using the initial coding scheme in Nvivo. We then relate these codes into themes and identify factors that concern users when they disclose their personal data into different software systems.
